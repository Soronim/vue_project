  ...
  methods: {
    async sendData() {
      try {
        ...
        // 3. Перенаправляем по роли (используем name маршрута)
        const routeName = userData.status === 'moderator' ? 'adminCabinet' : 'userCabinet';
        console.log('Redirecting to route:', routeName);
